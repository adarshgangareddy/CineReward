  ...
  try {
    ...
    for (const ticket of tickets) {
      if (ticket.userId && ticket.userId.email) {
        ...
        // 1. Email
        try {
          await emailService.sendReviewReminder(ticket.userId.email, ticket.movieTitle || 'your recent movie');
        ...
        if (ticket.userId.fcmTokens && ticket.userId.fcmTokens.length > 0) {
          ...
          const data = { ticketId: ticket._id.toString(), type: 'review_reminder' };
          
          sendPushNotification(ticket.userId.fcmTokens, title, body, data)
