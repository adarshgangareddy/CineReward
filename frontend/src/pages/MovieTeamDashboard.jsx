...
import { Film, Users, MessageSquare, TrendingUp, Award, Star, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const MovieTeamDashboard = () => {
  const [team] = useState(() => JSON.parse(localStorage.getItem('team')));
  const [selectedMovie, setSelectedMovie] = useState('');
  const [data, setData] = useState({ ticketCount: 0, recentReviews: [], pastReviewers: [] });
  ...
  const fetchMovieStats = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/admin/team/dashboard/${selectedMovie}?teamId=${team.id}`);
      setData(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const selectAiWinners = async () => {
    ...
    try {
      await axios.post('/api/admin/team/select-winners', { movieId: selectedMovie, teamId: team.id });
  ...
  const awardIndividual = async (reviewId) => {
    ...
    try {
      await axios.post('/api/admin/team/award-individual', { reviewId, teamId: team.id });
      await fetchMovieStats();
  ...
  const updateTicketCount = async () => {
    const count = prompt('Enter new free tickets count:', data.freeTicketsCount);
    if (count === null) return;
    try {
      await axios.post('/api/admin/team/update-tickets', { teamId: team.id, count: parseInt(count) });
      fetchMovieStats();
    } catch (err) {
      alert('Update failed');
    }
  ...
  if (!team) return <div className="text-center py-20 text-white">Unauthorized Access.</div>;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-4xl font-black text-white italic tracking-tighter">MOVIE<span className="text-primary">TEAM</span> HUB</h1>
            <p className="text-slate-500 font-medium">Analytics for {team.name}</p>
          </div>
        </div>
          ...
          <div className="relative flex-grow md:w-64">
            ...
            >
              {team.assignedMovies.map(mid => (
                <option key={mid} value={mid}>Movie ID: {mid}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-1 space-y-6">
           <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem] text-center hover:border-primary/30 transition-all">
             ...
             <p className="text-slate-500 text-xs font-black uppercase tracking-widest mb-1">Impact Factor</p>
             <p className="text-5xl font-black text-white tracking-tighter">
               {data.recentReviews.length + data.pastReviewers.length > 0 
                 ? ((data.recentReviews.reduce((acc, r) => acc + (r.rating || 0), 0) + data.pastReviewers.reduce((acc, r) => acc + (r.rating || 0), 0)) / (data.recentReviews.length + data.pastReviewers.length)).toFixed(1)
                 : '0.0'}
             </p>
             <p className="text-[10px] text-green-500 font-bold uppercase mt-4">Based on {data.recentReviews.length + data.pastReviewers.length} total reviews</p>
           </div>
           ...
           <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem] text-center">
             ...
             <p className="text-slate-500 text-xs font-black uppercase tracking-widest mb-1">Total Bookings</p>
             <p className="text-5xl font-black text-white tracking-tighter">{data.ticketCount}</p>
           </div>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden">
             <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                ...
                <div className="flex gap-3">
                  ...
                  <span className="text-slate-500 text-xs font-bold self-center">{data.recentReviews.length} recent</span>
                </div>
             </div>

             <div className="divide-y divide-slate-800">
               {data.recentReviews.map(review => (
                 <div key={review._id} className="p-8 hover:bg-slate-800/30 transition-all group">
                     ...
                     <div className="flex items-center gap-3">
                       <div className="w-10 h-10 bg-slate-950 rounded-full flex items-center justify-center text-xs font-black text-primary border border-slate-800 overflow-hidden">
                         {review.userId.profilePicture ? (
                           <img src={review.userId.profilePicture} alt="" className="w-full h-full object-cover" />
                         ) : (
                           review.userId.name ? review.userId.name[0] : 'U'
                         )}
                       </div>
                       <div>
                         <h4 className="text-white font-bold text-sm">{review.userId.name}</h4>
                         <p className="text-[10px] text-slate-500 font-bold">{review.userId.email}</p>
                       </div>
                     </div>
                     <div className="flex items-center gap-1.5 bg-yellow-500/10 px-3 py-1 rounded-full border border-yellow-500/20">
                       <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                       <span className="text-yellow-500 font-black text-xs">{review.rating}/10</span>
                     </div>
                   </div>
                   <p className="text-slate-400 text-sm leading-relaxed mb-6 italic">&quot;{review.comment}&quot;</p>
                   
                   <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">AI Relevance</span>
                        <div className="w-32 h-1.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800">
                           <div className="h-full bg-primary" style={{ width: `${review.aiScore}%` }}></div>
                        </div>
                        <span className="text-xs font-black text-primary">{review.aiScore}%</span>
                      </div>
          ...
          {data.pastReviewers.length > 0 && (
             <div ref={winnersRef} className="mt-12 scroll-mt-12">
               <div className="flex items-center gap-3 mb-8">
                 ...
                 <div>
                   <h3 className="text-2xl font-black text-white italic tracking-tighter uppercase">Past Reviewers</h3>
                   <p className="text-slate-500 text-xs font-bold">Successfully rewarded with coins</p>
                 </div>
               </div>

               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                 {data.pastReviewers.map(winner => (
                   <div key={winner._id} className="bg-slate-900 border border-green-500/20 p-6 rounded-3xl relative overflow-hidden group hover:border-green-500/40 transition-all">
                      ...
                      <div className="flex items-center gap-4 mb-4">
                        <div className="w-12 h-12 bg-slate-950 rounded-full flex items-center justify-center text-sm font-black text-green-500 border border-green-500/20 overflow-hidden">
                          {winner.userId?.profilePicture ? (
                            <img src={winner.userId.profilePicture} alt="" className="w-full h-full object-cover" />
                          ) : (
                            winner.userId?.name ? winner.userId.name[0] : 'U'
                          )}
                        </div>
                        <div className="max-w-[150px]">
                          <h4 className="text-white font-bold truncate">{winner.userId?.name || 'User'}</h4>
                          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest block truncate">{winner.userId?.email || 'No Email'}</span>
                        </div>
                      </div>
                      <p className="text-slate-400 text-xs italic leading-relaxed border-l-2 border-green-500/30 pl-4 py-1 line-clamp-2">&quot;{winner.comment}&quot;</p>
