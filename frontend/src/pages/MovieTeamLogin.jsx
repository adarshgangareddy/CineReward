...
import { Key, Film, Shield } from 'lucide-react';
...
const MovieTeamLogin = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('/api/admin/team/login', { secretKey });
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-950">
      <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] w-full max-w-lg shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase mb-2">Movie Team Access</h2>
          <p className="text-slate-500 text-sm font-medium uppercase tracking-widest">Restricted Internal Environment</p>
        </div>

        {error && <div className="bg-red-500/10 border border-red-500/50 text-red-500 p-4 rounded-xl mb-8 text-sm font-bold text-center">{error}</div>}

        <form onSubmit={handleLogin} className="space-y-6">

           <div className="space-y-2 pl-2">
             <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Secret Vault Key</label>
