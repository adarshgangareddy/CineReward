...
const Navbar = () => {
  ...
  const [selectedCity, setSelectedCity] = useState(
    localStorage.getItem("selectedCity") || "Bengaluru",
  );
    ...
    <nav className="bg-[#333545] text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center gap-8">
        <Link to="/" className="flex items-center gap-0 tracking-tighter">
          <span className="text-3xl font-black italic text-white uppercase">
            Cine
          </span>
          ...
          {user ? (
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 p-2 hover:bg-slate-700 rounded-xl transition-colors"
                >
                  <User className="w-5 h-5 text-primary" />
                  <ChevronDown className="w-4 h-4" />
                </button>
                {isAccountOpen && (
                  <div className="absolute right-0 top-12 z-40 w-52 bg-slate-900 border border-slate-700 rounded-xl p-2 shadow-2xl">
                    <Link
                      to="/dashboard"
                      onClick={() => setIsAccountOpen(false)}
                      className="block px-3 py-2 rounded-lg hover:bg-slate-800 text-sm"
                    >
                      My Dashboard
                    </Link>
                    <Link
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-800 text-sm text-red-300"
                    >
                      Logout
                    </button>
                  </div>
            ...
            <div className="flex items-center gap-4">
              <Link
                ...
                className="flex items-center gap-2 text-sm font-bold text-red-500 bg-red-500/10 px-3 py-1 rounded-lg border border-red-500/20"
              >
                <Shield className="w-5 h-5" /> Admin Hub
              </Link>
              <button
                onClick={handleLogout}
                className="text-slate-400 hover:text-primary transition-colors"
            ...
            <div className="flex items-center gap-4">
              <Link
                to="/list-your-show"
                className="hidden md:block text-slate-200 hover:text-primary transition-colors text-sm font-bold"
              >
                List Your Show
              </Link>
              <Link
      ...
      {isMenuOpen && (
        <div className="lg:hidden bg-slate-900 p-6 space-y-4 border-t border-slate-800">
          <SearchBar />
          <Link to="/" className="block py-2 border-b border-slate-800">
            Movies
          </Link>
          <Link
            to="/list-your-show"
            onClick={() => setIsMenuOpen(false)}
            className="block py-2 border-b border-slate-800"
          >
            List Your Show
          </Link>
          <Link
          ...
          <div className="bg-white rounded-xl w-full max-w-4xl overflow-hidden shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-8">
              ...
              <div className="relative mb-10 max-w-xl mx-auto">
                <SearchBar />
                <p className="text-center text-primary text-sm font-bold mt-4 cursor-pointer hover:underline">
                  Detect my location
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
                  {cities.map((city) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-bold transition-colors ${selectedCity === city.name ? "text-primary" : "text-slate-600 group-hover:text-primary"}`}
                      >
                        {city.name}
                      </span>
                    </button>
